...
Importing the two datasets pups and litters that we’re going to
manipulate.
...
Use `select()` to select variables by naming the data frame (litters_df)
and different variables you want to select.
...
You can also rename columns and rearrange columns (last 2 lines). Can
use relocate in place of select to rearrange columns.
...
Reorder the rows to ascending or descending order. You can also arrange
multiple rows. For example, arrange pups_born_alive first then gd0
weight.
